...
rem doi/scilista/existIssueDOI
...
then
	...
	cisis/mx seq=temp/doi/scilista/teste2_$1_$2.txt from=1 count=1 "pft=if v1<>'scilista' then v1/ fi'" now >> ../bases-work/doi/current_scilista/$1.txt
	cisis/mx seq=temp/doi/scilista/teste2_$1_$2.txt from=1 count=1 "pft=if v1='scilista' then '$1 $2'/ fi'" now >> ../bases-work/doi/scilista/$1.txt

	rm -rf temp/doi/scilista
fi
