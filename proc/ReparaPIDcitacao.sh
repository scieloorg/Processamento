...
# ------------------------------------------------------------------------- #
#      Entrada: Base de dados de revista no diretorio bases-work
...
cp -p ../bases-work/$1/$1.xrf ../chico/${1}.xrf
cp -p ../bases-work/$1/$1.xrf ../bases-work/$1/${1}.res.xrf
if [ "$?" -ne 0 ]
then
  exit 1
fi
cp -p ../bases-work/$1/$1.mst ../chico/${1}.mst
...
echo "Elimina a copia reserva da base da revista $1"
